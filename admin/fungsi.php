...
if (isset($_POST['tambahpetugas'])) {
    ...
    $cek    = mysqli_query($con, "SELECT * FROM login WHERE namapetugas='$namapetugas'");
    $hitung = mysqli_num_rows($cek);
    if ($hitung<1){
        if (in_array($ekstensi, $allowed_extension) === true) {
            if ($ukuran < 15000000) {
                move_uploaded_file($file_tmp, '../images/petugas/'.$foto);

                $query = mysqli_query($con, "INSERT INTO login (namapetugas, username, password, foto, role) VALUES ('$namapetugas','$username','$password','$foto','$role')");
                if ($query) {
                    echo'
...
if (isset($_POST['editpetugas'])) {
    ...
    if (in_array($ekstensi, $allowed_extension) == true) {
        if ($ukuran < 15000000) {
            ...
            $query = mysqli_query($con, "UPDATE login SET idlogin='$idlogin',namapetugas='$namapetugas',foto='$foto',username='$username',password='$password' WHERE idlogin='$idlogin'");
            if ($query) {
...
if (isset($_POST['hapuspetugas'])) {
    ...
    $query = mysqli_query($con, "DELETE FROM login WHERE idlogin='$idlogin'");
